...
class GraphObjectPagingLoader<T extends GraphObject> extends
		Loader<SimpleGraphObjectCursor<T>> {
	...
	public void followNextLink() {
		if (nextRequest != null) {
			...
			loading = true;
			final CacheableRequestBatch batch = putRequestIntoBatch(
					currentRequest, skipRoundtripIfCached);
	...
	private void startLoading(Request request, boolean skipRoundtripIfCached,
			long afterDelay) {
		...
		final RequestBatch batch = putRequestIntoBatch(request,
	...
	private void requestCompleted(Response response) {
		...
		FacebookException exception = (requestError == null) ? null
				: requestError.getException();
	...
	private void addResults(Response response) {
		final SimpleGraphObjectCursor<T> cursorToModify = (cursor == null || !appendResults) ? new SimpleGraphObjectCursor<T>()
				: new SimpleGraphObjectCursor<T>(cursor);

		final PagedResults result = response
				.getGraphObjectAs(PagedResults.class);
		final boolean fromCache = response.getIsFromCache();

		final GraphObjectList<T> data = result.getData().castToListOf(
				graphObjectClass);
		final boolean haveData = data.size() > 0;

		if (haveData) {
			nextRequest = response
					.getRequestForPagedResults(Response.PagingDirection.NEXT);
		...
		// Once we get any set of results NOT from the cache, stop trying to get
		// any future ones
		// from it.
		if (!fromCache) {
