...
class GraphObjectAdapter<T extends GraphObject> extends BaseAdapter implements
	...
	private final ArrayList<String> prefetchedProfilePictureIds = new ArrayList<String>();
	...
	public void prioritizeViewRange(int firstVisibleItem, int lastVisibleItem,
		...
		for (int i = lastVisibleItem; i >= 0; i--) {
			final SectionAndItem<T> sectionAndItem = getSectionAndItem(i);
			if (sectionAndItem.graphObject != null) {
				final String id = getIdOfGraphObject(sectionAndItem.graphObject);
				final ImageRequest request = pendingRequests.get(id);
				if (request != null) {
		...
		final int end = Math.min(lastVisibleItem + prefetchBuffer,
				getCount() - 1);
		...
		for (final T graphObject : graphObjectsToPrefetchPicturesFor) {
			final URI uri = getPictureUriOfGraphObject(graphObject);
			...
			final boolean alreadyPrefetching = prefetchedProfilePictureIds
					.remove(id);
	...
	protected URI getPictureUriOfGraphObject(T graphObject) {
		...
		if (o instanceof String) {
			...
			final ItemPicture itemPicture = GraphObject.Factory.create(
					(JSONObject) o).cast(ItemPicture.class);
	...
	protected View createGraphObjectView(T graphObject) {
		final View result = inflater.inflate(
				getGraphObjectRowLayoutId(graphObject), null);

		final ViewStub checkboxStub = (ViewStub) result
		...
		final ViewStub profilePicStub = (ViewStub) result
	...
	protected void populateGraphObjectView(View view, T graphObject) {
		...
		final CharSequence title = getTitleOfGraphObject(graphObject);
		...
		final CharSequence subtitle = getSubTitleOfGraphObject(graphObject);
		...
		if (getShowPicture()) {
			final URI pictureURI = getPictureUriOfGraphObject(graphObject);
			...
			if (pictureURI != null) {
				...
				if (prefetchedPictureCache.containsKey(id)) {
					final ImageResponse response = prefetchedPictureCache
							.get(id);
	...
	String getPictureFieldSpecifier() {
		...
		final View view = createGraphObjectView(null);
		final ImageView picture = (ImageView) view
				.findViewById(R.id.com_facebook_picker_image);
		if (picture == null) {
			return null;
		}
		...
		return String.format(Locale.US, "picture.height(%d).width(%d)",
	...
	private void rebuildSections() {
		...
		cursor.moveToFirst();
		do {
			final T graphObject = cursor.getGraphObject();
			...
			final String sectionKeyOfItem = getSectionKeyOfGraphObject(graphObject);
			...
			final List<T> section = graphObjectsBySection.get(sectionKeyOfItem);
		...
		if (sortFields != null) {
			...
			for (final List<T> section : graphObjectsBySection.values()) {
	...
	SectionAndItem<T> getSectionAndItem(int position) {
		...
		if (!displaySections) {
			...
			final List<T> section = graphObjectsBySection.get(sectionKey);
			...
			for (final String key : sectionKeys) {
				...
				final List<T> section = graphObjectsBySection.get(key);
	...
	int getPosition(String sectionKey, T graphObject) {
		...
		for (final T t : graphObjectsBySection.get(sectionKey)) {
	...
	public boolean isEnabled(int position) {
		final SectionAndItem<T> sectionAndItem = getSectionAndItem(position);
	...
	public List<T> getGraphObjectsById(Collection<String> ids) {
		...
		for (final String id : idSet) {
			final T graphObject = graphObjectsById.get(id);
			if (graphObject != null) {
		...
		if (imageView == null) {
			...
			if (response.getBitmap() != null) {
				...
				if (prefetchedPictureCache.size() >= MAX_PREFETCHED_PICTURES) {
					...
					final String oldestId = prefetchedProfilePictureIds
							.remove(0);
	...
	private static int compareGraphObjects(GraphObject a, GraphObject b,
		...
		for (final String sortField : sortFields) {
			...
			final String sb = (String) b.getProperty(sortField);
