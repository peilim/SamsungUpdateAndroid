...
public class FacebookDialog {
	...
	/**
	 * Provides an interface for describing a specific feature provided by a
	 * FacebookDialog. This is public primarily to allow its use elsewhere in
	 * the Android SDK; developers are discouraged from constructing their own
	 * DialogFeature implementations internal API may change.
	 */
	public interface DialogFeature {
		 ...
		 * This method is for internal use only.
		 *
		 * For all Enums that implement this interface, the name() method is
		 * already present. It returns the String representation of the Enum
		 * value, verbatim.
		 *
		 */
		String name();
	...
	public static boolean handleActivityResult(Context context,
		...
		if (callback != null) {
			if (NativeProtocol.isErrorResult(data)) {
				final Bundle errorData = NativeProtocol
						.getErrorDataFromResultIntent(data);
				final Exception error = NativeProtocol
						.getExceptionFromErrorData(errorData);

				callback.onError(appCall, error, errorData);
			} else {
				final Bundle successResults = NativeProtocol
						.getSuccessResultsFromIntent(data);
	...
	private static boolean handleCanPresent(Context context,
		...
		final String actionName = getActionForFeatures(features);
	...
	private static int[] getVersionSpecForFeatures(String applicationId,
			String actionName, Iterable<? extends DialogFeature> features) {
		int[] intersectedRange = null; // Null is treated as a fully open Range.
										// So it is safe to compare against.
		for (final DialogFeature feature : features) {
			final int[] featureVersionSpec = getVersionSpecForFeature(
					applicationId, actionName, feature);
			intersectedRange = Utility.intersectRanges(intersectedRange,
					featureVersionSpec);
		}
	...
	private static int[] getVersionSpecForFeature(String applicationId,
			String actionName, DialogFeature feature) {
		// Return the value from DialogFeatureConfig if available. Otherwise,
		// just default to the min-version
		final Utility.DialogFeatureConfig config = Utility
				.getDialogFeatureConfig(applicationId, actionName,
						feature.name());
		if (config != null) {
	...
	private static void logDialogActivity(Activity activity, Fragment fragment,
		...
		final AppEventsLogger logger = AppEventsLogger
	...
	static private String getEventName(Intent intent) {
		...
		final Bundle extras = intent
				.getBundleExtra(NativeProtocol.EXTRA_PROTOCOL_METHOD_ARGS);
		if (extras != null) {
			final ArrayList<String> photo = extras
					.getStringArrayList(NativeProtocol.METHOD_ARGS_PHOTOS);
			final String video = extras
					.getString(NativeProtocol.METHOD_ARGS_VIDEO);
	...
	/**
	 * Provides a base class for various FacebookDialog builders. This is public
	 * primarily to allow its use elsewhere in the Android SDK; developers are
	 * discouraged from constructing their own FacebookDialog builders as the
	 * internal API may change.
	 *
	 * @param <CONCRETE>
	 *            The concrete base class of the builder.
	 */
	public abstract static class Builder<CONCRETE extends Builder<?>> {
		...
		public FacebookDialog build() {
			...
			final String action = getActionForFeatures(getDialogFeatures());
			final int protocolVersion = getProtocolVersionForNativeDialog(
					activity,
					action,
					getVersionSpecForFeatures(applicationId, action,
							getDialogFeatures()));

			Bundle extras = null;
			if (NativeProtocol
					.isVersionCompatibleWithBucketedIntent(protocolVersion)) {
			...
			final Intent intent = NativeProtocol.createPlatformActivityIntent(
					activity, appCall.getCallId().toString(), action,
					protocolVersion, applicationName, extras);
			if (intent == null) {
				logDialogActivity(
						activity,
						fragment,
		 ...
		 * SDK; developers are discouraged from consuming this method as the
		 * internal API may change.
		 */
		protected String getWebFallbackUrlInternal() {
			final Iterable<? extends DialogFeature> features = getDialogFeatures();
			...
			Uri fallbackUrl;
			if (config == null
					|| (fallbackUrl = config.getFallbackUrl()) == null) {
				return null;
			}

			// Since we're talking to the server here, let's use the latest
			// version we know about.
			// We know we are going to be communicating over a bucketed
			// protocol.
			final Bundle methodArguments = getMethodArguments();
			final int protocolVersion = NativeProtocol.getLatestKnownVersion();
			final Bundle webParams = ServerProtocol
					.getQueryParamsForPlatformActivityIntentWebFallback(
							activity, appCall.getCallId().toString(),
							protocolVersion, applicationName, methodArguments);
			if (webParams == null) {
			...
			if (fallbackUrl.isRelative()) {
				fallbackUrl = Utility.buildUri(
						...
						fallbackUrl.toString(), webParams);
			}
		...
		OnPresentCallback getOnPresentCallback() {
			return new OnPresentCallback() {
				...
				public void onPresent(Context context) throws Exception {
					...
					if (mediaAttachmentFiles != null
							&& mediaAttachmentFiles.size() > 0) {
						getAttachmentStore().addAttachmentFilesForCall(context,
								appCall.getCallId(), mediaAttachmentFiles);
					}
				}
			};
		}
		...
		protected List<String> addImageAttachments(Collection<Bitmap> bitmaps) {
			...
			for (final Bitmap bitmap : bitmaps) {
				...
				final String url = NativeAppCallContentProvider
		...
		protected String addVideoAttachmentFile(File videoFile) {
			...
			final String url = NativeAppCallContentProvider.getAttachmentUrl(
	...
	private abstract static class ShareDialogBuilderBase<CONCRETE extends ShareDialogBuilderBase<?>>
		...
		/**
		 * Sets the name of the URL to be shared. This method only has effect if
		 * setLink is called.
		 *
		 * @param name
		 *            the name
		...
		/**
		 * Sets the caption of the URL to be shared. This method only has effect
		...
		/**
		 * Sets the description of the URL to be shared. This method only has
		...
		/**
		 * Sets the URL of the image of the URL to be shared. This method only
		 * has effect if setLink is called.
		 *
		 * @param picture
		...
		public CONCRETE setFriends(List<String> friends) {
			this.friends = (friends == null ? null : new ArrayList<String>(
					friends));
			@SuppressWarnings("unchecked")
			final CONCRETE result = (CONCRETE) this;
			return result;
		...
		protected Bundle getMethodArguments() {
			final Bundle methodArguments = new Bundle();

			putExtra(methodArguments, NativeProtocol.METHOD_ARGS_TITLE, name);
			putExtra(methodArguments, NativeProtocol.METHOD_ARGS_SUBTITLE,
					caption);
			putExtra(methodArguments, NativeProtocol.METHOD_ARGS_DESCRIPTION,
					description);
			...
			putExtra(methodArguments, NativeProtocol.METHOD_ARGS_PLACE_TAG,
					place);
			putExtra(methodArguments, NativeProtocol.METHOD_ARGS_REF, ref);

			methodArguments.putBoolean(
					NativeProtocol.METHOD_ARGS_DATA_FAILURES_FATAL,
					dataErrorsFatal);
			if (!Utility.isNullOrEmpty(friends)) {
				methodArguments.putStringArrayList(
						NativeProtocol.METHOD_ARGS_FRIEND_TAGS, friends);
			}
	...
	private static abstract class PhotoDialogBuilderBase<CONCRETE extends PhotoDialogBuilderBase<?>>
		...
		private String place;
		private ArrayList<String> friends;
		private final ArrayList<String> imageAttachmentUrls = new ArrayList<String>();
		...
		protected Bundle getMethodArguments() {
			...
			methodArgs.putStringArrayList(NativeProtocol.METHOD_ARGS_PHOTOS,
					imageAttachmentUrls);

			if (!Utility.isNullOrEmpty(friends)) {
				methodArgs.putStringArrayList(
						NativeProtocol.METHOD_ARGS_FRIEND_TAGS, friends);
			}
	...
	public static class PhotoMessageDialogBuilder extends
		...
		/**
		 * setPlace is not supported for the photo message dialog, setting this
		...
		/**
		 * setFriends is not supported for the photo message dialog, setting
	...
	private static abstract class VideoDialogBuilderBase<CONCRETE extends VideoDialogBuilderBase<?>>
		...
		/**
		 * Adds a video to display in the native Share dialog, by specifying the
		 * location in the file system. The dialog's callback will be called
		 * once the user has shared the video, but the video may be uploaded in
		 * the background by the Facebook app; apps wishing to be notified when
		 * the video upload has succeeded or failed should extend the
		 * FacebookBroadcastReceiver class and register it in their
		 * AndroidManifest.xml.
		 * 
		 * @param video
		 *            a Files representing the video to be uploaded
		 * @return this instance of the builder
		 */
		...
		protected Bundle getMethodArguments() {
			final Bundle methodArgs = new Bundle();
			putExtra(methodArgs, NativeProtocol.METHOD_ARGS_PLACE_TAG, place);
			methodArgs.putString(NativeProtocol.METHOD_ARGS_VIDEO,
					videoAttachmentUrl);
			return methodArgs;
		}
	}
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Share dialog for sharing videos. This builder
	 * will throw an exception if the Facebook application is not installed, so
	 * it should only be used if
	 * {@link FacebookDialog#canPresentShareDialog(android.content.Context, com.facebook.widget.FacebookDialog.ShareDialogFeature...)}
	 * indicates the capability is available.
	 */
	public static class VideoShareDialogBuilder extends
			VideoDialogBuilderBase<VideoShareDialogBuilder> {
		/**
		 * Constructor.
		 *
		 * @param activity
	...
	public static class MessageDialogBuilder extends
		...
		/**
		 * setPlace is not supported for the message dialog, setting this method
		 * will have no effect.
		 *
		 * @param place
		 *            will be ignored
		...
		/**
		 * setFriends is not supported for the message dialog, setting this
		 * method will have no effect.
		 *
		 * @param friends
		 *            will be ignored
	...
	private static abstract class OpenGraphDialogBuilderBase<CONCRETE extends OpenGraphDialogBuilderBase<?>>
		...
		public OpenGraphDialogBuilderBase(Activity activity,
			...
			final String typeOnAction = action.getType();
		...
		public CONCRETE setImageAttachmentsForAction(List<Bitmap> bitmaps,
			...
			final List<String> attachmentUrls = addImageAttachments(bitmaps);
		...
		public CONCRETE setImageAttachmentFilesForAction(
			...
			final List<String> attachmentUrls = addImageAttachmentFiles(bitmapFiles);
			updateActionAttachmentUrls(attachmentUrls, isUserGenerated);
		...
		protected Bundle getMethodArguments() {
			...
			putExtra(methodArgs,
					...
					previewPropertyName);
			putExtra(methodArgs, NativeProtocol.METHOD_ARGS_ACTION_TYPE,
					actionType);
			methodArgs.putBoolean(
					NativeProtocol.METHOD_ARGS_DATA_FAILURES_FATAL,
					dataErrorsFatal);
