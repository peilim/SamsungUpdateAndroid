...
public class FriendPickerFragment extends PickerFragment<GraphUser> {
	...
	/**
	 * The key for a String parameter in the fragment's Intent bundle to
	 * indicate the type of friend picker to use. This value is case sensitive,
	 * and must match the enum @{link FriendPickerType}
	 */
	...
	public enum FriendPickerType {
		FRIENDS("/friends", true), TAGGABLE_FRIENDS("/taggable_friends", false), INVITABLE_FRIENDS(
				"/invitable_friends", false);
	 ...
	 * 
	 * @param multiSelect
	 *            true if the user can select multiple friends, false if only
	 *            one friend
	 */
	...
	/**
	 * Sets the list of friends for pre selection. These friends will be
	 * selected by default.
	 * 
	 * @param userIds
	...
	PickerFragmentAdapter<GraphUser> createAdapter() {
		final PickerFragmentAdapter<GraphUser> adapter = new PickerFragmentAdapter<GraphUser>(
				this.getActivity()) {
	...
	SelectionStrategy createSelectionStrategy() {
		return multiSelect ? new MultiSelectionStrategy()
	...
	Request getRequestForLoadData(Session session) {
		if (adapter == null) {
			throw new FacebookException(
					"Can't issue requests until Fragment has been created.");
		}

		final String userToFetch = (userId != null) ? userId : "me";
	...
	void logAppEvents(boolean doneButtonClicked) {
		...
		logger.logSdkEvent(AnalyticsEvents.EVENT_FRIEND_PICKER_USAGE, null,
				parameters);
	}
	...
	private Request createRequest(String userID, Set<String> extraFields,
			Session session) {
		final Request request = Request.newGraphPathRequest(session, userID
				+ friendPickerType.getRequestPath(), null);
	...
	private void setFriendPickerSettingsFromBundle(Bundle inState) {
		...
		if (inState != null) {
			...
			setMultiSelect(inState.getBoolean(MULTI_SELECT_BUNDLE_KEY,
					multiSelect));
			if (inState.containsKey(FRIEND_PICKER_TYPE_KEY)) {
				try {
					friendPickerType = FriendPickerType.valueOf(inState
							.getString(FRIEND_PICKER_TYPE_KEY));
	...
	private class ImmediateLoadingStrategy extends LoadingStrategy {
		...
		protected void onLoadFinished(
				GraphObjectPagingLoader<GraphUser> loader,
			...
			if (data.areMoreObjectsAvailable()) {
				...
				// If this was from the cache, schedule a delayed refresh query
				// (unless we got no results
				// at all, in which case refresh immediately.
				if (data.isFromCache()) {
					loader.refreshOriginalRequest(data.getCount() == 0 ? CACHED_RESULT_REFRESH_DELAY
							: 0);
				}
			}
		}
		...
		private void followNextLink() {
			// This may look redundant, but this causes the circle to be
			// alpha-dimmed if we have results.
