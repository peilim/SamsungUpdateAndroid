...
public class Session implements Serializable {
	...
	Session(Context context, String applicationId,
		...
		final Bundle tokenState = loadTokenFromCache ? tokenCachingStrategy
				.load() : null;
		if (TokenCachingStrategy.hasTokenInformation(tokenState)) {
			final Date cachedExpirationDate = TokenCachingStrategy.getDate(
	...
	public boolean isPermissionGranted(String permission) {
		final List<String> grantedPermissions = getPermissions();
	...
	static PermissionsPair handlePermissionResponse(Response response) {
		...
		final GraphMultiResult result = response
		...
		final List<String> grantedPermissions = new ArrayList<String>(
				data.size());
		final List<String> declinedPermissions = new ArrayList<String>(
				data.size());
		...
		if (firstObject.getProperty("permission") != null) { // v2.0
			for (final GraphObject graphObject : data) {
				final String permission = (String) graphObject
						.getProperty("permission");
				if (permission.equals("installed")) {
					continue;
				}
				final String status = (String) graphObject
						.getProperty("status");
			...
			for (final Map.Entry<String, Object> entry : permissionsMap
					.entrySet()) {
	...
	public static final Session restoreSession(Context context,
		...
		if (data != null) {
			...
			try {
				final Session session = (Session) (new ObjectInputStream(is))
						.readObject();
				initializeStaticContext(context);
	...
	static void initializeStaticContext(Context currentContext) {
		if ((currentContext != null) && (staticContext == null)) {
			final Context applicationContext = currentContext
	...
	private void logAuthorizationStart() {
		...
		final AppEventsLogger logger = getAppEventsLogger();
	...
	private void logAuthorizationComplete(
		...
		if (pendingAuthorizationRequest == null) {
			...
			if (resultExtras != null) {
				...
				try {
					for (final Map.Entry<String, String> entry : resultExtras
							.entrySet()) {
	...
	private boolean tryLoginActivity(AuthorizationRequest request) {
		final Intent intent = getLoginActivityIntent(request);
	...
	private boolean resolveIntent(Intent intent) {
		final ResolveInfo resolveInfo = getStaticContext().getPackageManager()
	...
	private Intent getLoginActivityIntent(AuthorizationRequest request) {
		...
		final AuthorizationClient.AuthorizationRequest authClientRequest = request
				.getAuthorizationClientRequest();
		final Bundle extras = LoginActivity
				.populateIntentExtras(authClientRequest);
	...
	class TokenRefreshRequest implements ServiceConnection {
		...
		public void bind() {
			final Intent intent = NativeProtocol
	...
	static class TokenRefreshRequestHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			final String token = msg.getData().getString(
					AccessToken.ACCESS_TOKEN_KEY);
			final Session session = sessionWeakReference.get();

			if (session != null && token != null) {
				session.extendTokenCompleted(msg.getData());
			}

			final TokenRefreshRequest request = refreshRequestWeakReference
					.get();
			if (request != null) {
	...
	public static final class NewPermissionsRequest extends
		...
		@Override
		AuthorizationClient.AuthorizationRequest getAuthorizationClientRequest() {
			final AuthorizationClient.AuthorizationRequest request = super
					.getAuthorizationClientRequest();
