...
public class BundleJSONConverter {
	...
	static {
		...
		SETTERS.put(String[].class, new Setter() {
			...
			public void setOnJSON(JSONObject json, String key, Object value)
				...
				for (final String stringValue : (String[]) value) {
		...
		SETTERS.put(JSONArray.class, new Setter() {
			...
			public void setOnBundle(Bundle bundle, String key, Object value)
				...
				// Empty list, can't even figure out the type, assume an
				// ArrayList<String>
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					final Object current = jsonArray.get(i);
			...
			public void setOnJSON(JSONObject json, String key, Object value)
					throws JSONException {
				throw new IllegalArgumentException(
						"JSONArray's are not supported in bundles.");
			}
		});
	}
	...
	public static JSONObject convertToJSON(Bundle bundle) throws JSONException {
		...
		for (final String key : bundle.keySet()) {
			...
			// Special case List<String> as getClass would not work, since List
			// is an interface
			if (value instanceof List<?>) {
				final JSONArray jsonArray = new JSONArray();
				@SuppressWarnings("unchecked")
				final List<String> listValue = (List<String>) value;
				for (final String stringValue : listValue) {
					jsonArray.put(stringValue);
				}
				json.put(key, jsonArray);
				continue;
			...
			// Special case Bundle as it's one way, on the return it will be
			// JSONObject
			...
			final Setter setter = SETTERS.get(value.getClass());
			if (setter == null) {
	...
	public static Bundle convertToBundle(JSONObject jsonObject)
		...
		@SuppressWarnings("unchecked")
		final Iterator<String> jsonIterator = jsonObject.keys();
		while (jsonIterator.hasNext()) {
			...
			// Special case JSONObject as it's one way, on the return it would
			// be Bundle.
