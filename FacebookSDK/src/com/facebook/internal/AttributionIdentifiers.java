...
public class AttributionIdentifiers {
	...
	private static AttributionIdentifiers getAndroidId(Context context) {
		...
		try {
			// We can't call getAdvertisingIdInfo on the main thread or the app
			// will potentially
			// freeze, if this is the case throw:
			if (Looper.myLooper() == Looper.getMainLooper()) {
				throw new FacebookException(
						"getAndroidId cannot be called on the main thread.");
			}
			final Method isGooglePlayServicesAvailable = Utility
					.getMethodQuietly(
			...
			final Object connectionResult = Utility.invokeMethodQuietly(null,
			...
			final Method getAdvertisingIdInfo = Utility
					.getMethodQuietly(
			...
			final Object advertisingInfo = Utility.invokeMethodQuietly(null,
					getAdvertisingIdInfo, context);
			if (advertisingInfo == null) {
				return identifiers;
			}

			final Method getId = Utility.getMethodQuietly(
					advertisingInfo.getClass(), "getId");
			final Method isLimitAdTrackingEnabled = Utility.getMethodQuietly(
	...
	public static AttributionIdentifiers getAttributionIdentifiers(
		...
		try {
			...
			c = context.getContentResolver().query(ATTRIBUTION_ID_CONTENT_URI,
					projection, null, null, null);
			if (c == null || !c.moveToFirst()) {
				return identifiers;
			}
			final int attributionColumnIndex = c
					.getColumnIndex(ATTRIBUTION_ID_COLUMN_NAME);
			final int androidIdColumnIndex = c
					.getColumnIndex(ANDROID_ID_COLUMN_NAME);
			final int limitTrackingColumnIndex = c
