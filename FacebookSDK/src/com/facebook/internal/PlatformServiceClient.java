...
abstract public class PlatformServiceClient implements ServiceConnection {
	...
	public boolean start() {
		...
		final int availableVersion = NativeProtocol
		...
		final Intent intent = NativeProtocol
				.createPlatformServiceIntent(context);
		if (intent == null) {
			return false;
		} else {
			running = true;
	...
	private void sendMessage() {
		...
		final Message request = Message.obtain(null, requestMessage);
	...
	private void callback(Bundle result) {
		...
		final CompletedListener callback = listener;
		if (callback != null) {
