...
public final class FacebookRequestError {
	...
	private FacebookRequestError(int requestStatusCode, int errorCode,
			...
			String errorUserTitle, String errorUserMessage,
			boolean errorIsTransient, JSONObject requestResultBody,
		...
		shouldNotify = errorUserMessage != null
	...
	private FacebookRequestError(int requestStatusCode, int errorCode,
		...
		this(requestStatusCode, errorCode, subErrorCode, errorType,
				...
				errorIsTransient, requestResultBody, requestResult,
	...
	public FacebookRequestError(int errorCode, String errorType,
		...
		this(INVALID_HTTP_STATUS_CODE, errorCode, INVALID_ERROR_CODE,
				errorType, errorMessage, null, null, false, null, null, null,
				null, null);
	}
	...
	/**
	 * A message suitable for display to the user, describing a user action
	 * necessary to enable Facebook functionality. Not all Facebook errors yield
	 * a message suitable for user display; however in all cases where
	 * shouldNotifyUser() returns true, this method returns a non-null message
	...
	/**
	 * A short summary of the error suitable for display to the user. Not all
	 * Facebook errors yield a title/message suitable for user display; however
	 * in all cases where getErrorUserTitle() returns valid String - user should
	 * be notified.
	 *
	 * @return the error message returned from Facebook
	 */
	...
	/**
	 * @return true if given error is transient and may succeed if the initial
	 *         action is retried as-is. Application may use this information to
	 *         display a "Retry" button, if user should be notified about this
	 *         error.
	 */
	...
	static FacebookRequestError checkResponseAndCreateError(
		...
		try {
			if (singleResult.has(CODE_KEY)) {
				...
				final Object body = Utility.getStringPropertyAsJSON(
				...
				if (body != null && body instanceof JSONObject) {
					...
					if (jsonBody.has(ERROR_KEY)) {
						...
						errorSubCode = error.optInt(ERROR_SUB_CODE_KEY,
								INVALID_ERROR_CODE);
						errorUserMessage = error.optString(ERROR_USER_MSG_KEY,
								null);
						errorUserTitle = error.optString(ERROR_USER_TITLE_KEY,
								null);
						errorIsTransient = error.optBoolean(
								ERROR_IS_TRANSIENT_KEY, false);
						hasError = true;
					...
					if (hasError) {
						return new FacebookRequestError(responseCode,
								...
								errorMessage, errorUserTitle, errorUserMessage,
								errorIsTransient, jsonBody, singleResult,
