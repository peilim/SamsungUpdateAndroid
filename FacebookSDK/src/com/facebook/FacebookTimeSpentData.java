...
class FacebookTimeSpentData implements Serializable {
	...
	private static final long[] INACTIVE_SECONDS_QUANTA = new long[] {
			5 * DateUtils.MINUTE_IN_MILLIS, 15 * DateUtils.MINUTE_IN_MILLIS,
			30 * DateUtils.MINUTE_IN_MILLIS, 1 * DateUtils.HOUR_IN_MILLIS,
			6 * DateUtils.HOUR_IN_MILLIS, 12 * DateUtils.HOUR_IN_MILLIS,
			1 * DateUtils.DAY_IN_MILLIS, 2 * DateUtils.DAY_IN_MILLIS,
			3 * DateUtils.DAY_IN_MILLIS, 7 * DateUtils.DAY_IN_MILLIS,
			14 * DateUtils.DAY_IN_MILLIS, 21 * DateUtils.DAY_IN_MILLIS,
			28 * DateUtils.DAY_IN_MILLIS, 60 * DateUtils.DAY_IN_MILLIS,
			90 * DateUtils.DAY_IN_MILLIS, 120 * DateUtils.DAY_IN_MILLIS,
			150 * DateUtils.DAY_IN_MILLIS, 180 * DateUtils.DAY_IN_MILLIS,
	...
	/**
	 * Serialization proxy for the FacebookTimeSpentData class. This is version
	 * 1 of serialization. Future serializations may differ in format. This
	 * class should not be modified. If serializations formats change, create a
	 * new class SerializationProxyVx.
	 */
	private static class SerializationProxyV1 implements Serializable {
		...
		SerializationProxyV1(long lastResumeTime, long lastSuspendTime,
				long millisecondsSpentInSession, int interruptionCount) {
		...
		private Object readResolve() {
			return new FacebookTimeSpentData(lastResumeTime, lastSuspendTime,
					millisecondsSpentInSession, interruptionCount);
		}
	}
	 ...
	 */
	private FacebookTimeSpentData(long lastResumeTime, long lastSuspendTime,
			long millisecondsSpentInSession, int interruptionCount
	...
	/**
	 * Serialization proxy for the FacebookTimeSpentData class. This is version
	 * 2 of serialization. Future serializations may differ in format. This
	 * class should not be modified. If serializations formats change, create a
	 * new class SerializationProxyVx.
	 */
	private static class SerializationProxyV2 implements Serializable {
		...
		SerializationProxyV2(long lastResumeTime, long lastSuspendTime,
				long millisecondsSpentInSession, int interruptionCount,
	...
	private Object writeReplace() {
		return new SerializationProxyV2(lastResumeTime, lastSuspendTime,
				millisecondsSpentInSession, interruptionCount,
				firstOpenSourceApplication);
	}
	...
	void onSuspend(AppEventsLogger logger, long eventTime) {
		if (!isAppActive) {
			Logger.log(LoggingBehavior.APP_EVENTS, TAG,
					"Suspend for inactive app");
			return;
		}

		final long now = eventTime;
	...
	void onResume(AppEventsLogger logger, long eventTime,
		...
		// Retain old behavior for activated app event - log the event if the
		// event hasn't
		// been logged in the previous suppression interval or this is a cold
		// launch.
		// If this is a cold launch, always log the event. Otherwise, use the
		// last
		// event log time to determine if the app activate should be suppressed
		// or not.
		if (isColdLaunch()
		...
		// If this is an application that's not calling onSuspend yet, log and
		// return. We can't
		// track time spent for this application as there are no calls to
		// onSuspend.
		if (isAppActive) {
		...
		long interruptionDurationMillis = wasSuspendedEver() ? now
				- lastSuspendTime : 0;
		...
		// If interruption duration is > new session threshold, then log old
		// session
		...
		if (interruptionDurationMillis > NUM_MILLISECONDS_IDLE_TO_BE_NEW_SESSION) {
			...
			// We're not logging this resume event - check to see if this should
			// count
			// as an interruption
		...
		// Set source application only for the first resume of the timespent
		// session.
	...
	private void logAppDeactivatedEvent(AppEventsLogger logger,
		...
		final Bundle eventParams = new Bundle();
		eventParams.putInt(AppEventsConstants.EVENT_NAME_SESSION_INTERRUPTIONS,
				interruptionCount);
		eventParams.putString(
				AppEventsConstants.EVENT_NAME_TIME_BETWEEN_SESSIONS, String
						.format("session_quanta_%d",
	...
	private boolean isColdLaunch() {
		...
		final boolean result = !isWarmLaunch;
		isWarmLaunch = true;
		return result;
	}
}
