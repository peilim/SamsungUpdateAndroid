...
class AuthorizationClient implements Serializable {
	...
	boolean checkInternetPermission() {
		...
		final int permissionCheck = checkPermission(Manifest.permission.INTERNET);
		if (permissionCheck != PackageManager.PERMISSION_GRANTED) {
			final String errorType = context
					.getString(R.string.com_facebook_internet_permission_error_title);
			final String errorDescription = context
	...
	void tryNextHandler() {
		...
		while (handlersToTry != null && !handlersToTry.isEmpty()) {
			...
			final boolean started = tryCurrentHandler();
	...
	boolean tryCurrentHandler() {
		...
		final boolean tried = currentHandler.tryAuthorize(pendingRequest);
		if (tried) {
	...
	void validateSameFbidAndFinish(Result pendingResult) {
		...
		final RequestBatch batch = createReauthValidationBatch(pendingResult);
	...
	RequestBatch createReauthValidationBatch(final Result pendingResult) {
		...
		final String validateSameFbidAsToken = pendingRequest
				.getPreviousAccessToken();
		final Request requestCurrentTokenMe = createGetProfileIdRequest(validateSameFbidAsToken);
		requestCurrentTokenMe.setCallback(meCallback);

		final Request requestNewTokenMe = createGetProfileIdRequest(newToken);
		requestNewTokenMe.setCallback(meCallback);

		final Request requestCurrentTokenPermissions = createGetPermissionsRequest(validateSameFbidAsToken);
		requestCurrentTokenPermissions.setCallback(new Request.Callback() {
			@Override
			public void onCompleted(Response response) {
				try {
					final Session.PermissionsPair permissionsPair = Session
		...
		final RequestBatch batch = new RequestBatch(requestCurrentTokenMe,
			...
			public void onBatchCompleted(RequestBatch batch) {
				try {
					...
					if (fbids.size() == 2 && fbids.get(0) != null
						...
						final AccessToken tokenWithPermissions = AccessToken
	...
	private void logAuthorizationMethodStart(String method) {
		final Bundle bundle = newAuthorizationLoggingBundle(pendingRequest
				.getAuthId());
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			if (!Utility.isNullOrEmpty(request.getPermissions())) {
				final String scope = TextUtils.join(",",
						request.getPermissions());
				parameters.putString(ServerProtocol.DIALOG_PARAM_SCOPE, scope);
				addLoggingExtra(ServerProtocol.DIALOG_PARAM_SCOPE, scope);
			}

			final SessionDefaultAudience audience = request
					.getDefaultAudience();
			...
			final WebDialog.Builder builder = new AuthDialogBuilder(
			...
			if (values != null) {
				...
				if (error instanceof FacebookOperationCanceledException) {
					...
					if (error instanceof FacebookServiceException) {
						final FacebookRequestError requestError = ((FacebookServiceException) error)
								.getRequestError();
		...
		private void saveCookieToken(String token) {
			final Context context = getStartActivityDelegate()
					.getActivityContext();
		...
		private String loadCookieToken() {
			...
			final SharedPreferences sharedPreferences = context
					.getSharedPreferences(WEB_VIEW_AUTH_HANDLER_STORE,
			...
			if (result != null) {
				...
				if ((currentPermissions != null)
					...
					final AccessToken token = AccessToken
							.createFromNativeLogin(
									result,
									AccessTokenSource.FACEBOOK_APPLICATION_SERVICE);
					final Result outcome = Result.createTokenResult(
	...
	class KatanaProxyAuthHandler extends KatanaAuthHandler {
		...
		boolean tryAuthorize(AuthorizationRequest request) {
			...
			final Intent intent = NativeProtocol.createProxyAuthIntent(context,
	...
	private void logWebLoginCompleted(String applicationId, String e2e) {
		final AppEventsLogger appEventsLogger = AppEventsLogger.newLogger(
	...
	static class AuthDialogBuilder extends WebDialog.Builder {
		...
		public WebDialog build() {
			final Bundle parameters = getParameters();
	...
	static class Result implements Serializable {
		...
		static Result createErrorResult(AuthorizationRequest request,
			...
			final String message = TextUtils.join(": ",
