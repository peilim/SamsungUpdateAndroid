...
public class Facebook {
	...
	public void authorize(Activity activity, String[] permissions,
		...
		final SessionLoginBehavior behavior = (activityCode >= 0) ? SessionLoginBehavior.SSO_WITH_FALLBACK
	...
	private void authorize(Activity activity, String[] permissions,
		...
		final Session.OpenRequest openRequest = new Session.OpenRequest(
				activity).setCallback(callback).setLoginBehavior(behavior)
	...
	private void onSessionCallback(Session callbackSession, SessionState state,
		...
		final Bundle extras = callbackSession.getAuthorizationBundle();
	...
	public void authorizeCallback(int requestCode, int resultCode, Intent data) {
		...
		final Session pending = this.pendingOpeningSession;
		if (pending != null) {
	...
	private static class TokenRefreshConnectionHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			final Facebook facebook = facebookWeakReference.get();
			final TokenRefreshServiceConnection connection = connectionWeakReference
					.get();
			...
			final long expiresAtMsecFromEpoch = msg.getData().getLong(EXPIRES) * 1000L;

			if (token != null) {
				facebook.setAccessToken(token);
				facebook.setAccessExpires(expiresAtMsecFromEpoch);

				final Session refreshSession = facebook.session;
	...
	public final Session getSession() {
		while (true) {
			...
			final Session newSession = new Session.Builder(
	...
	public void setAccessExpiresIn(String expiresInSecsFromNow) {
		...
		if (expiresInSecsFromNow != null) {
			final long expires = expiresInSecsFromNow.equals("0") ? 0 : System
					.currentTimeMillis()
